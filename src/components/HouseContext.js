...
const HouseContextProvider = ({ children }) => {
  const [appartments, setAppartments] = useState(housesData);
  ...
  useEffect(() => {
    ...
    const allCountries = appartments.map((house) => {
  ...
  useEffect(() => {
    ...
    const allProperties = appartments.map((house) => {
      return house.type;
    });
  ...
  useEffect(() => {
    ...
    const allCategories = appartments.map((house) => {
  ...
  const handleClick = () => {
    ...
    setTimeout(() => {
      return (
        newHouses.length < 1 ? setAppartments([]) : setAppartments(newHouses),
        setLoading(false)
      );
    }, 1000);
  };
